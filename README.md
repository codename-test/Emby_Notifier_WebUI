...
| 无图模板优化 | ❌ 始终请求 TMDB | ✅ **无图模板跳过 TMDB，直接推送** |
| TMDB 失败处理 | ❌ 丢弃消息 | ✅ **自动切换回退模板，不丢消息** |
| 消息格式适配 | ❌ 需手动选择服务器类型 | ✅ **智能识别 Emby/Jellyfin 四种格式** |
...
| 模板 | 类型 | 说明 |
|------|------|------|
| **标准** | 普通 | 带封面图，完整 TMDB 数据（简介、评分、链接） |
| **基础模板** | 回退 | TMDB 失败时自动使用，无图，仅 webhook 自带数据 |

### 模板规则

- **回退模板**：系统专用，不可删除、不可在端口下拉中选择、封面图强制关闭
- **无图模板**：`enable_image=0` 时自动跳过 TMDB 请求，直接用 webhook 数据推送（节省时间、避免 TMDB 超时）
- **TMDB 失败**：使用标准模板但 TMDB 获取失败时，自动切换回退模板推送，**不丢消息**
